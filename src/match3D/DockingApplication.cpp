...
{
  m_impl = std::make_shared<DockingAppImpl>();
  glfwSetErrorCallback(glfw_error_callback);
}

void DockingApplication::run(int width, int height, const char *name)
{
  m_impl->width = width;
...
bool DockingApplication::getWindowSize(int &width, int &height) const
{
  width = m_impl->width;
...
void DockingAppImpl::init()
{
  ...
  glfwSetFramebufferSizeCallback(
      window, [](GLFWwindow *w, int newWidth, int newHeight) {
        auto *app = (DockingAppImpl *)glfwGetWindowUserPointer(w);
        app->width = newWidth;
...
void DockingAppImpl::renderWindows()
{
  for (auto &w : windows)
    w->renderUI();
}
